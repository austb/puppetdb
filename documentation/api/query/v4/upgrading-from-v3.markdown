...
[v4-catalogs]: ./catalogs.markdown
[v4-factsets]: ./factsets.markdown
[v1-mbeans]: ../../metrics/v1/mbeans.markdown
...
* (3.0) The v4 catalogs endpoint has caused the response of the
  ...
  see [/pdb/query/v4/catalogs documentation](./catalogs.markdown).
...
* (2.0.0) The v4 catalogs endpoint does not contain a `metadata` field
  ...
  information, see the
  [/pdb/query/v4/catalogs documentation][v4-catalogs].
...
* (2.2) The v4 factsets endpoint was added to facilitate the grouping
  ...
  [/pdb/query/v4/factsets documentation][v4-factsets].
...
* (3.0) The `facts` field is now expanded as per our new expansion
  ...
  the [/pdb/query/v4/factsets documentation][v4-factsets].
...
* (3.0) the `aggregate-event-counts` endpoint now accepts multiple `summarize_by`
  ...
  documentation](./aggregate_event_counts).
...
* (3.0) The former metrics endpoint has been split off into a separate service, and
  ...
  http://localhost:8080/metrics/v1/mbeans and so on, according to the [metrics api documentation][v1-mbeans].
...
#### /pdb/cmd/v1 (formerly /v3/commands)
  ...
  * (3.0) For users posting commands directly to the
    ...
    versions will be
    [replace catalogs v6](../../wire_format/catalog_format_v6.markdown),
    [store report v5](../../wire_format/report_format_v5.markdown),
    .markdown [replace facts v4](../../wire_format/facts_format_v4.markdown).
...
* (2.2.0) `/pdb/query/v4/factsets` This endpoint returns a key-value
  ...
  [/pdb/query/v4/factsets documentation](./factsets.markdown#response-format).
...
* (3.0) `/pdb/query/v4/reports/<hash>/metrics` This endpoint allows
  ...
  [/pdb/query/v4/reports documentation](./reports.markdown)
...
* (3.0) `/pdb/query/v4/catalogs/<node>/[resources|edges]` Both of
  ...
  [/pdb/query/v4/catalogs documentation](./catalogs.markdown)

#### Features affecting all endpoints

* (3.0) Extract is available as a top-level query operator, useful for selecting only
  certain fields from a response. See the [documentation on the extract operator](./ast.markdown#extract) for more information.
...
* (3.0) The v4 events endpoint does not require a query parameter, so
  ...
  [events endpoint documentation](./events.markdown#pdbqueryv4events)
  for more information.

#### /pdb/query/v4/reports

* (3.0) The response of the reports endpoint includes the new fields `noop`,
  `environment`, `status`, `resource_events`, `logs`, and `metrics`. For more information, see the [documentation on the reports endpoint](./reports.markdown). For comparison, see [an example of the new format](./reports.markdown#examples), and [an example of the old format](/puppetdb/2.3/api/query/v3/reports.html#response-format) (PuppetDB 2.3 docs).
...
* (3.0) The reports endpoint takes a `latest_report?` query to return only reports
  ...
  For more information, see the [documentation on the report query fields](./reports.markdown#query-fields).
...
* (2.2.0) The new `select_fact_contents` subquery operator allows for filtering the
  ...
  values. This is exhibited on the bottom of the [subquery examples documentation](./ast.markdown#explicit-subquery-examples).

* (2.2.0) We have added the regexp array match operator `~>` for querying fact paths on
  the `fact-contents` or `fact-paths endpoints`. This is documented with the other [operators](./ast.markdown#regexp-array-match).
  An example of usage is given at the bottom of the [subquery examples page](./ast.markdown#explicit-subquery-examples).

* (3.0) We have added the `group_by` and `function` operators, as well as
  support for the `count` function. For more information, see the [operators documentation](./ast.markdown#function).
